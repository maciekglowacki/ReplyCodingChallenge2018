...
def read_input():
    ...
    for provider in range(providers_count):
        ...
        for region in range(int(regions_count)):
            ...
            services_per_package = {k: v for k, v in zip(services, services_per_package)}
            latency = [int(x) for x in input().split()]
            latency = {k: v for k, v in zip(countries, latency)}
            regions.append(Region(region_name, latency, PackageInfo(
                packages_count, package_cost, services_per_package)))
